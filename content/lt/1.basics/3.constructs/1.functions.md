...
```python line-numbers=false
def sum_digits(n: float|int) -> int
```
...
```python line-numbers=false
def sort_list(numbers: list[int|float]) -> list[int|float]
```

### 8. Savaitės pardavimų analizė

Faile `sales_data.csv`{ lang=text } saugomi duomenys apie parduotus produktų kiekius per savaitę. Šį failą galite atsiųsti čia.
...
Duomenis galite atsisiųsti čia. Čia rasite dvylikos savaičių failus, kurių formatas vienodas. Failo pavyzdys:
