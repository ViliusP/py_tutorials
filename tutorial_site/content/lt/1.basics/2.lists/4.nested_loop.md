...
Šis pavyzdyje bus naudojami programavimo principai, kurių dar neturėtumėte mokėti. Jeigu norėsite sekti šį pavyzdį, susikurkite tekstinį failą *pixels.txt* tame pačiame aplankale, kuriame bus jūsų kodas. Taip pat bus panaudota biblioteką *matplotlib*. Įsirašyti galite suvedę į terminalą:

```console
pip3 install matplotlib
```

Į failą *pixels.txt*, įkelkite duomenis, kuriuos rasite puslapio apačioje. Turint duomenis pirmiausia reikės nuskaiyti. Tai išmoksite kitoje pamokoje, bet dabar bandykite sekti pavyzdį:
...
Toliau kodo nebūtina aiškintis, eilutės žemiau reikalingos, kad būtų galima pamatyti vizualų rezultatą ir pokytį tarp pradinio ir rezultato paveiksliuko. Tam naudojama mūsų įsirašyta *matplotlib* biblioteka:
...
<!-- ```text file=<rootDir>/pixel_data.txt
``` -->